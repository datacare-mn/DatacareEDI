...
namespace EDIWEBAPI.Utils
{
    public  class IMSUtils
    {
       // readonly ILogger<HttpRestUtils> _log;
        ...
        public IMSUtils(OracleDbContext dbcontext)
        {
            try
            {
                _dbcontext = dbcontext;
                HttpModel = Logics.IMSLogic.GetHttpModelData(_dbcontext, 2);
                Token = PrepareToken(2, _dbcontext);
        ...
        public  string PrepareToken(int comid, OracleDbContext _dbcontext)
        {
            try
            {
                ...
                if (DateTime.Now > expTime)
                {
                    ...
                    if (TokenData != null)
                    {
                        Logics.IMSLogic.UpdateAppToken(_dbcontext, comid, TokenData);
            ...
            try
            {
                if (Token != null)
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        //client.BaseAddress = new Uri("http://10.150.10.20:8890");
        ...
        public async Task<ResponseClient> Delete(string geturl)
        {
            try
            {
                if (Token != null)
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", Token);
                        HttpResponseMessage response = await client.DeleteAsync(geturl);
                        if (response.IsSuccessStatusCode)
                        {
                            Task<string> data = response.Content.ReadAsStringAsync();
                            JObject rss = JObject.Parse(data.Result.ToString());
        ...
        public async Task<ResponseClient> Put(string posturl, string postdata)
        {
            try
            {
                if (Token == null)
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", Token);
                    var response = await client.PutAsync(posturl, content);
                    if (response.IsSuccessStatusCode)
                    {
                        Task<string> data = response.Content.ReadAsStringAsync();
                        JObject rss = JObject.Parse(data.Result.ToString());
        ...
        public async Task<ResponseClient> PutForm(string posturl, MultipartFormDataContent postdata)
        {
            try
            {
                if (Token == null)
                ...
                {
                    client.BaseAddress = new Uri(HttpModel.BaseApi);
                    //client.BaseAddress = new Uri("http://10.150.10.44:8890");
                    client.DefaultRequestHeaders.Accept.Clear();
                    //client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", Token);
                    var response = await client.PutAsync(posturl, postdata);
                    if (response.IsSuccessStatusCode)
                    {
                        Task<string> data = response.Content.ReadAsStringAsync();
                        JObject rss = JObject.Parse(data.Result.ToString());
